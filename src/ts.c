...
unsigned char ayc_read_ts(const char *tsfile, unsigned char *probedata)
{
   ...
   if (len > 1 << 16)
   {
      const char *append = "_ayc.ts";
      char probetsfilename[255] = "";
         ...
         if (strlen(tsfile) < sizeof(probetsfilename) - strlen(append))
         {
            ...
            if (tmp)
            {
               *tmp = 0;
               ...
               const char *msg = "\x47\1\1\1\x0d\x0aThis file was generated by AYCWABTU\x0d\x0a";
               memset(probefile + PCKTSIZE * 3, 32, PCKTSIZE);
               memcpy(probefile + PCKTSIZE * 3, msg, strlen(msg));
