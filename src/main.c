...
// Cache line aligment to avoid false sharing
#define _ALIGNED_TYPE(t,x) typedef ThreadInfo ALIGNED_(16)
...
void process_block_of_keys(void *arg){
   ...
   int benchmark = info->benchmark;
   dvbcsa_bs_word_t r[8 * (1 + 8 + 56)]; // working data block
   unsigned char *probedata = info->probedata;
   dvbcsa_bs_word_t *bs_data_sb0 = info->bs_data_sb0;
